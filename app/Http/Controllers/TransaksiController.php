...
class TransaksiController extends Controller
{
    public function index(Request $request)
    {
        ...
        $transaksis = Transaksi::join('members', 'members.id', 'transaksis.member_id')
            ...
            ->when($search, function ($query, $search) {
                return $query->where('members.nama', 'like', "%{$search}%");
            })
    ...
    public function add(Request $request, Member $member)
    {
        ...
        Cart::session($member->id)->add(array(
            ...
            'attributes' => [
                ...
                'diskon' => $paket->diskon,
                'discount_type' =>$paket->discount_type
    ...
    public function update(Request $request, Transaksi $transaksi)
    {
        ...
        $query_transaksi = [
            'tgl_bayar' => $uang_tunai ? date('Y-m-d H:i:s') : null,
            'tgl_selesai' => ($transaksi->status == 'selesai') ? date('Y-m-d H:i:s') : null,
            'tgl_diambil' => ($transaksi->status == 'diambil') ? date('Y-m-d H:i:s') : null,
            'tgl_diantar' => ($transaksi->status == 'diantar') ? date('Y-m-d H:i:s') : null,
            'biaya_tambahan' => $biaya_tambahan,
            'diskon' => $diskon,
            'pajak' => $pajak,
            'sub_total' => $subtotal,
    ...
    const ALLOWED_VALUES = ['baru', 'proses', 'selesai', 'diambil','batal','diantar'];
    public function status(Transaksi $transaksi, $status)
    {
        // $allowedValue = ['baru','proses','selesai','diambil','batal'];

        // if(!in_array($status, $allowedValue)) {
        ...
        if($status == 'selesai' || $status == 'diambil' || $status == 'diantar'){
            $transaksi->update([
                'tgl_'.$status => Carbon::now(),
            ]);
        }

        LogActivity::add('berhasil mengupdate status transaksi');
        return back()->with('message', 'success update');
    }
    ...
    public function invoice(Transaksi $transaksi)
    {
        ...
        $items = TransaksiDetail::join('pakets', 'pakets.id', 'transaksi_details.paket_id')
            ...
            LogActivity::add('berhasil mencetak invoice');
